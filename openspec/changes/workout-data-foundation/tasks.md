...
- [ ] 0.3 Create the PR 1 branch `feat/workout-data-foundation-s1-schema-write-path` from the tracker branch. Sync Notion F0.1 `Status` to `En curso` with `Fase / Referencia` = `openspec/changes/workout-data-foundation/` (per `.claude/rules/notion-backlog.md` (read-only)).
